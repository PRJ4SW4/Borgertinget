...
namespace backend.Services
{
    public class DailySelectionService : IDailySelectionService
    {
        ...
        public async Task<FakePolitiker?> GetOrSelectDailyPoliticianAsync(GamemodeTypes gameMode)
        {
            ...
            _logger.LogDebug(
                "Attempting to get daily politician for {GameMode} on {Date}",
                gameMode,
                today
            );

            var existingSelection = await _context
                .DailySelections.Include(ds => ds.SelectedPolitiker)
            ...
            if (existingSelection?.SelectedPolitiker != null)
            {
                _logger.LogInformation(
                    "Found existing daily selection for {GameMode} on {Date}: PolitikerId {PolitikerId}",
                    gameMode,
                    today,
                    existingSelection.SelectedPolitikerID
                );
                // Sikkerhedstjek for Include
                if (
                    existingSelection.SelectedPolitiker.FakeParti == null
                    && existingSelection.SelectedPolitiker.PartiId > 0
                )
                {
                    _logger.LogWarning(
                        "Party not included for existing selection's politician {PolitikerId}. Attempting manual load.",
                        existingSelection.SelectedPolitikerID
                    );
                    existingSelection.SelectedPolitiker.FakeParti =
                        await _context.FakePartier.FindAsync(
                            existingSelection.SelectedPolitiker.PartiId
                        );
                }
            ...
            _logger.LogInformation(
                "No existing selection found for {GameMode} on {Date}. Performing weighted selection.",
                gameMode,
                today
            );
            try
            {
                ...
                {
                    _logger.LogWarning(
                        "Weighted selection for {GameMode} returned no politician.",
                        gameMode
                    );
                    return null;
                }
                ...
                {
                    _logger.LogError(
                        "Party was unexpectedly null after PerformWeightedSelectionAsync for politician {PolitikerId}. Attempting manual load.",
                        selectedPolitician.Id
                    );
                    selectedPolitician.FakeParti = await _context.FakePartier.FindAsync(
                        selectedPolitician.PartiId
                    );
                    if (selectedPolitician.FakeParti == null)
                    {
                        _logger.LogError(
                            "Could not find party with ID {PartiId} for selected politician {PolitikerId} after manual load.",
                            selectedPolitician.PartiId,
                            selectedPolitician.Id
                        );
                ...
                try
                {
                    ...
                    _logger.LogInformation(
                        "Successfully selected and saved PolitikerId {PolitikerId} for {GameMode} on {Date}",
                        selectedPolitician.Id,
                        gameMode,
                        today
                    );
                ...
                catch (Exception ex)
                {
                    ...
                    _logger.LogError(
                        ex,
                        "Error saving daily selection or updating tracking for {GameMode}.",
                        gameMode
                    );
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Error during weighted selection process call for {GameMode}.",
                    gameMode
                );
                throw; // Kast fejlen videre
            }
        ...
        {
            _logger.LogInformation(
                "Fetching politicians for guessing with search term: '{SearchTerm}'",
                search ?? "<null>"
            );
            ...
            _logger.LogInformation(
                "Returning {Count} politician summaries for search term: '{SearchTerm}'",
                politicians.Count,
                search ?? "<null>"
            );
            return politicians;
        ...
        public async Task<GuessResultDto> ProcessGuessAsync(GuessRequestDto guessDto)
        {
            ...
            if (correctPolitician == null)
            {
                throw new KeyNotFoundException(
                    $"Ingen dagens politiker fundet for spiltype {guessDto.GameMode}."
                );
            }

            // 2. Hent gættet politiker (m/ parti)
            var guessedPolitician = await _context
                .FakePolitikere.Include(p => p.FakeParti)
                .AsNoTracking()
            ...
            int correctAge = CalculateAge(
                DateOnly.FromDateTime(correctPolitician.DateOfBirth),
                today
            );
            int guessedAge = CalculateAge(
                DateOnly.FromDateTime(guessedPolitician.DateOfBirth),
                today
            );
            // ****************************************************
            ...
            {
                result.Feedback["Navn"] = result.IsCorrectGuess
                    ? FeedbackType.Korrekt
                    : FeedbackType.Forkert;
                result.Feedback["Parti"] =
                    correctPolitician.PartiId == guessedPolitician.PartiId
                        ? FeedbackType.Korrekt
                        : FeedbackType.Forkert;

                // Sammenlign beregnede aldre
                if (correctAge == guessedAge)
                    result.Feedback["Alder"] = FeedbackType.Korrekt;
                else if (correctAge > guessedAge)
                    result.Feedback["Alder"] = FeedbackType.Højere;
                else
                    result.Feedback["Alder"] = FeedbackType.Lavere;

                result.Feedback["Region"] = string.Equals(
                    correctPolitician.Region,
                    guessedPolitician.Region,
                    StringComparison.OrdinalIgnoreCase
                )
                    ? FeedbackType.Korrekt
                    : FeedbackType.Forkert;
                result.Feedback["Køn"] = string.Equals(
                    correctPolitician.Køn,
                    guessedPolitician.Køn,
                    StringComparison.OrdinalIgnoreCase
                )
                    ? FeedbackType.Korrekt
                    : FeedbackType.Forkert;
                result.Feedback["Uddannelse"] = string.Equals(
                    correctPolitician.Uddannelse,
                    guessedPolitician.Uddannelse,
                    StringComparison.OrdinalIgnoreCase
                )
                    ? FeedbackType.Korrekt
                    : FeedbackType.Forkert;
            ...
            _logger.LogInformation(
                "Guess result calculated for GuessedId {GuessedId}: IsCorrect={IsCorrect}, FeedbackItems={FeedbackCount}",
                guessDto.GuessedPoliticianId,
                result.IsCorrectGuess,
                result.Feedback.Count
            );
        ...
        {
            DateOnly today = DateOnly.FromDateTime(DateTime.UtcNow);
            var selection = await _context
                .DailySelections.AsNoTracking()
                .FirstOrDefaultAsync(ds =>
                    ds.SelectionDate == today && ds.GameMode == GamemodeTypes.Citat
                );
            if (selection == null)
            {
                throw new KeyNotFoundException($"Ingen dagens citat fundet for {today}.");
            }
            if (string.IsNullOrEmpty(selection.SelectedQuoteText))
            {
                throw new InvalidOperationException($"Intet citat gemt for {today}.");
            }
        ...
        public async Task<PhotoDto> GetPhotoOfTheDayAsync()
        {
            ...
            var photoData = await _context
                .FakePolitikere.Where(p => p.Id == politicianId)
                .Select(p => p.Portræt)
                .FirstOrDefaultAsync();
            if (photoData == null)
            {
                throw new KeyNotFoundException(
                    $"Politiker ({politicianId}) fundet i DailySelection, men ikke i FakePolitikere, eller Portræt er null."
                );
            }
            if (photoData.Length == 0)
            {
                throw new InvalidOperationException(
                    $"Politiker ({politicianId}) har tomme portræt data."
                );
            }
        ...
        public async Task SelectAndSaveDailyPoliticiansAsync(DateOnly date)
        {
            ...
            bool alreadyExists = await _context.DailySelections.AnyAsync(ds =>
                ds.SelectionDate == date
            );
            if (alreadyExists)
            {
                _logger.LogWarning(
                    "Daily selections already exist for {Date}. Skipping generation.",
                    date
                );
                return;
            }
            ...
            var allPoliticians = await _context
                .FakePolitikere.Include(p => p.Quotes)
                .Include(p => p.GameTrackings)
                .ToListAsync();
            if (!allPoliticians.Any())
            {
                _logger.LogError("No politicians found.");
                return;
            }
            ...
            var classicCandidates = allPoliticians
                .Select(p =>
                    (
                        politician: p,
                        weight: CalculateSelectionWeight(p, GamemodeTypes.Klassisk, date)
                    )
                )
                .ToList();
            ...
            {
                _logger.LogError("Could not select classic politician.");
                return;
            }
            ...
            var quoteCandidatesRaw = allPoliticians
                .Where(p => p.Quotes != null && p.Quotes.Any())
                .ToList();
            if (!quoteCandidatesRaw.Any())
            {
                _logger.LogWarning(
                    "No politicians with quotes found. Selecting random politician for Quote mode fallback."
                );
                var fallbackQuoteCandidates = allPoliticians
                    .Select(p =>
                        (
                            politician: p,
                            weight: CalculateSelectionWeight(p, GamemodeTypes.Citat, date)
                        )
                    )
                    .ToList();
            ...
            else
            {
                ...
                {
                    selectedQuote = quotePolitician.Quotes.ElementAt(
                        _random.Next(quotePolitician.Quotes.Count)
                    );
                }
            }
            if (quotePolitician == null)
            {
                _logger.LogError("Could not select quote politician.");
                return;
            }

            // Vælg Foto (med fallback)
            FakePolitiker? photoPolitician = null;
            var photoCandidatesRaw = allPoliticians
                .Where(p => p.Portræt != null && p.Portræt.Length > 0)
                .ToList();
            if (!photoCandidatesRaw.Any())
            {
                _logger.LogWarning(
                    "No politicians with portraits found. Selecting random politician for Photo mode fallback."
                );
                var fallbackPhotoCandidates = allPoliticians
                    .Select(p =>
                        (
                            politician: p,
                            weight: CalculateSelectionWeight(p, GamemodeTypes.Foto, date)
                        )
                    )
                    .ToList();
            ...
            {
                _logger.LogError("Could not select photo politician.");
                return;
            }
            ...
            await _context.SaveChangesAsync();
            _logger.LogInformation(
                "Successfully selected and saved daily politicians for {Date}. Classic: {ClassicId}, Quote: {QuoteId}, Photo: {PhotoId}",
                date,
        ...
        private async Task<int> GetSelectedPoliticianIdAsync(GamemodeTypes gameMode, DateOnly today)
        {
            ...
            {
                throw new KeyNotFoundException(
                    $"Ingen dagens politiker fundet for {gameMode} d. {today}."
                );
            }
        ...
        private async Task<FakePolitiker?> PerformWeightedSelectionAsync(
            GamemodeTypes gameMode,
            DateOnly today
        )
        {
            const int maxWeightDays = 365 * 2;
            const int defaultWeight = maxWeightDays + 1;
            _logger.LogDebug(
                "Performing weighted selection for {GameMode} on {Date}",
                gameMode,
                today
            );

            var candidates = await _context
                .FakePolitikere.Include(p => p.FakeParti) // Inkluder parti
            ...
            if (!candidates.Any())
            {
                _logger.LogWarning("No candidates found for weighted selection...");
                return null;
            }
            ...
            var weightedCandidates = candidates
                .Select(c =>
                {
                    ...
                    int daysSinceLast =
                        tracker?.LastSelectedDate.HasValue ?? false
                            ? today.DayNumber - tracker.LastSelectedDate.Value.DayNumber
                            : defaultWeight;
            ...
            return SelectWeightedRandom(
                weightedCandidates.Select(wc => (wc.Politician, wc.Weight)).ToList()
            );
        }

        // CalculateSelectionWeight (som før)
        private int CalculateSelectionWeight(
            FakePolitiker politician,
            GamemodeTypes gameMode,
            DateOnly currentDate
        )
        {
            const int maxWeight = 365 * 2;
            ...
            {
                int days = (currentDate.DayNumber - tracker.LastSelectedDate.Value.DayNumber);
                return Math.Max(baseWeight, Math.Min(maxWeight, days + baseWeight));
            }
        }

        // SelectWeightedRandom (som før)
        private FakePolitiker? SelectWeightedRandom(
            List<(FakePolitiker politician, int weight)> weightedCandidates
        )
        {
            ...
            _logger.LogError("Weighted random selection failed unexpectedly...");
            return validCandidates.LastOrDefault().politician; // Fallback
        }
        ...
        {
            var existingTracker = politician.GameTrackings?.FirstOrDefault(gt =>
                gt.GameMode == gameMode
            );
            if (existingTracker == null)
            {
                existingTracker = await _context.GameTrackings.FirstOrDefaultAsync(gt =>
                    gt.PolitikerId == politician.Id && gt.GameMode == gameMode
                );
            }
            if (existingTracker != null)
            {
            ...
            {
                var nt = new PolidleGamemodeTracker
                {
                    PolitikerId = politician.Id,
                    GameMode = gameMode,
                    LastSelectedDate = selectionDate,
                };
                await _context.GameTrackings.AddAsync(nt);
            }

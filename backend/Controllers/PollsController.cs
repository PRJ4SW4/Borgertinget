    ...
    public class PollsController : ControllerBase
    {
        ...
        public async Task<ActionResult<PollDetailsDto>> CreatePoll(CreatePollDto createPollDto)
        {
            var politician = await _context
                ...
                .FirstOrDefaultAsync(p =>
                    p.TwitterUserId == createPollDto.PoliticianTwitterId.ToString()
                );
            if (politician == null)
            {
        ...
        public async Task<ActionResult<PollDetailsDto>> GetPollById(int id)
        {
            ...
            var poll = await _context
                .Polls.Include(p => p.Options.OrderBy(o => o.Id)) // Sorter options her
        ...
        public async Task<ActionResult<IEnumerable<PollSummaryDto>>> GetPolls()
        {
            var polls = await _context
                .Polls.Select(p => new PollSummaryDto { Id = p.Id, Question = p.Question })
                .ToListAsync();
        ...
        public async Task<IActionResult> Vote(int pollId, VoteDto voteDto)
        {
            ...
            bool alreadyVoted = await _context.UserVotes.AnyAsync(uv =>
                uv.UserId == currentUserId && uv.PollId == pollId
            );
            if (alreadyVoted)
            {
                return Conflict("Du har allerede stemt på denne afstemning.");
            }
            ...
            try
            {
                ...
                var updatedOptionsData = poll
                    .Options.OrderBy(o => o.Id) // Sorter for konsistens
                ...
                Console.WriteLine(
                    $"User {currentUserId} voted for option {voteDto.OptionId} on poll {pollId}. SignalR broadcast sent."
                ); // Log success
            }
            ...
            return new PollDetailsDto
            {
                ...
                Options =
                    poll.Options? // Brug null-conditional operator
                        ...
                        .OrderBy(o => o.Id)
                        .ToList() ?? new List<PollOptionDto>(), // Returner tom liste hvis options er null

...
namespace backend.Data
{
    public class PolidleSeed
    {
        ...
        // Portræt-filnavne
        private static readonly List<string> MalePortraits = new List<string>
        {
            "mand1.png",
            "mand2.png",
            "mand3.png",
        ...
        private static readonly List<string> FemalePortraits = new List<string>
        {
            "kvinde1.png",
            "kvinde2.png",
            "kvinde3.png",
        ...
        private static async Task<byte[]> GetPortraitBytesAsync(string gender, string imageBasePath)
        {
            ...
            {
                Console.WriteLine(
                    $"ADVARSEL: Portrætfil ikke fundet: {fullImagePath}. Prøver default."
                );
            ...
            if (File.Exists(fullImagePath))
            {
                ...
                {
                    Console.WriteLine(
                        $"FEJL: Kunne ikke læse billedfil {fullImagePath}: {ex.Message}"
                    );
            ...
            else
            {
                if (selectedFileName == DefaultPortrait)
                    Console.WriteLine($"ADVARSEL: Default portrætfil ikke fundet: {fullImagePath}");
                return Array.Empty<byte>();
            }
        }
        ...
        public static async Task SeedDataAsync(DataContext context, int numberOfPoliticians = 75)
        {
            ...
            if (!await context.FakePolitikere.AnyAsync())
            {
                ...
                {
                    Console.WriteLine(
                        "FEJL: Kunne ikke finde nogen partier i DB at tilknytte til politikere. Politker-seeding afbrydes."
                    );
                    return;
                }
                Console.WriteLine(
                    $"INFO: Fundet {seededPartier.Count} partier at bruge til politiker-seeding."
                );
                ...
                for (int i = 0; i < numberOfPoliticians; i++)
                {
                    ...
                    DateTime birthDateTimeUtc = birthDate.ToDateTime(
                        TimeOnly.MinValue,
                        DateTimeKind.Utc
                    );
                    // ================================================================
                ...
                Console.WriteLine(
                    $"INFO: {numberOfPoliticians} tilfældige politikere seeded (med portrætter, partiId, DateOfBirth og citater)."
                );
            }
            else
            {
            ...
            // Log samlet status
            Console.WriteLine(
                $"INFO: Seeding status - Parties seeded: {partiesWereSeeded}, Politicians seeded: {politiciansWereSeeded}"
            );
        }
    }
}

...
import {DailyPoliticianDto, GuessRequestDto, GuessResultDto, PhotoDto, QuoteDto, SearchListDto,} from '../types/PolidleTypes';  // Sørg for korrekt sti til din types fil
...
async function handleApiResponse<T>(response: Response): Promise<T> {
  if (!response.ok) {
    ...
    console.error(
        'API Kald Fejlede:', errorMessage, 'Original response:', response);
 ...
 */
export const fetchPoliticiansForSearch =
    async(search?: string): Promise<SearchListDto[]> => {
...
export const submitGuess =
    async(guessData: GuessRequestDto): Promise<GuessResultDto> => {
  const url = `${API_BASE_URL}/guess`;
  _logger.LogDebug(
      'Submitting guess to URL: {url} with data: {@guessData}', url, guessData);
  const response = await fetch(url, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',

...
import type {FlashcardCollectionDetailDto, FlashcardCollectionSummaryDto} from '../types/flashcardTypes';
import type {PageDetailDto, PageSummaryDto} from '../types/pageTypes'; // Import types
...
export const fetchCalendarEvents = async(
    startDate?: string, endDate?: string): Promise<CalendarEventDto[]> => {
  ...
  const url =
      `${API_BASE_URL}/calendar/events${queryString ? `?${queryString}` : ''}`;

...
function PageContent() {
  ...
  const handleAnswerChange = (questionId: number, answerOptionId: number) => {
    ...
    if (
      feedback[questionId] === undefined ||
      feedback[questionId] === null ||
      feedback[questionId] === "pending"
    ) {
  ...
  if (error)
    return (
      <div style={{ color: "red" }}>Fejl ved indlæsning af side: {error}</div>
    );
  if (!pageDetails)
    return <div>Siden blev ikke fundet eller data mangler.</div>;
  ...
  return (
    <article className="page-content-article">
      ...
      {/* --- Question Sections --- */}
      {pageDetails.associatedQuestions &&
        pageDetails.associatedQuestions.length > 0 && (
          <div className="all-questions-container">
            {pageDetails.associatedQuestions.map((question) => {
              ...
              const isDisabled =
                isCorrectlyAnswered || isPending || isIncorrectAndWait;
              const canSubmit = !(
              ...
              return (
                <section
                  key={question.id}
                  className={`question-section feedback-${
                  ...
                  >
                    ...
                    <fieldset className="answer-options" disabled={isDisabled}>
                      <legend className="sr-only">
                        Svar muligheder for spørgsmål {question.id}
                      </legend>
                    ...
                    <div className="feedback-message">
                      {currentFeedback === "correct" && (
                        <p className="correct">✅ Korrekt!</p>
                      )}
                      {currentFeedback === "incorrect" && (
                        <p className="incorrect">❌ Forkert.</p>
                      )}
                      {currentFeedback === "pending" && (
                        <p className="pending">Tjekker svar...</p>
                      )}
                    </div>
                    {/* --- End Feedback --- */}
      ...
      <div className="page-navigation-buttons">
        {pageDetails.previousSiblingId ? (
          <Link
            to={`/learning/${pageDetails.previousSiblingId}`}
            className="page-nav-button prev"
          >
            &lt; Forrige side
          </Link>
        ) : (
          <span className="page-nav-button disabled prev">
            &lt; Forrige side
          </span>
        )}
        {pageDetails.nextSiblingId ? (
          <Link
            to={`/learning/${pageDetails.nextSiblingId}`}

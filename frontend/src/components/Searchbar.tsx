...
const SearchBar: React.FC = () => {
  ...
  const fetchSuggestions = useCallback(async (query: string) => {
    if (isFullSearchActive) { // If a full search is active, don't fetch suggestions.
  ...
  }, [isFullSearchActive]); // isFullSearchActive is a key dependency
  ...
  const getResultLink = (result: SearchDocument): string => {
    ...
    switch (result.dataType.toLowerCase()) {
      case 'aktor': return `/politician/${actualId}`;
      case 'flashcard':
        return result.collectionId ? `/flashcards/${result.collectionId}#flashcard-${actualId}` : '/flashcards';
      case 'party': return `/party/${encodeURIComponent(result.partyName || 'unknown')}`;
      case 'page': return `/learning/${actualId}`;
      default: return '#';
    }
  };
  ...
  const getDataTypeDisplayName = (dataType: string): string => {
    switch (dataType.toLowerCase()) {
      case 'aktor': return 'Politiker';
      case 'flashcard': return 'Flashcard';
      case 'party': return 'Parti';
      case 'page': return 'Læringsside';
      default: return dataType;
    }
  };

  const shouldShowSearchFeedback = isLoading || error ||
    ...
    <div className="search-component-container" ref={searchContainerRef}>
      ...
      {showSuggestions && searchQuery.trim() && !isFullSearchActive && (
        <ul className="search-suggestions-list">
          ...
          {!isSuggestionsLoading && searchSuggestions.length > 0 && searchSuggestions.map((suggestion, index) => (
            <li
              key={index}
          ...
          {!isSuggestionsLoading && searchSuggestions.length === 0 && (
              <li className="suggestion-item no-suggestions">Ingen forslag fundet.</li>
          )}
        </ul>
      )}
            ...
            <ul className="search-results-list main-results">
              {searchResults.map((result) => (
                <li key={result.id} className="search-result-item">
                  <Link to={getResultLink(result)} className="search-result-link" onClick={() => {
                      setShowSuggestions(false);
                    ...
                    <span className="result-title">
                      {result.title || result.aktorName || result.collectionTitle || result.pageTitle || result.partyName || result.frontText || result.backText || 'Ukendt Titel'}
                    </span>
                  </Link>
                </li>
              ))}
